...
def process(im):
    ...
    for i in contours:
        ...
        if (bw > bw_width):
            cv2.line(im, (bx, by), (bx + bw, by + bh), (0, 255, 0), 2)  # draw the a contour line
    ...
    # run ransac for LEFT
    model_ransac = linear_model.RANSACRegressor(linear_model.LogisticRegression())
...
cap = cv2.VideoCapture('images/inputVideo.mp4')  # load a video
#
W = cap.get(3)  # get width
H = cap.get(4)  # get height
# frame = cv2.imread("images/main2.jpg")
# W, H, _ = frame.shape
...
while (cap.isOpened()):
    ...
    try:
        ...
        print "Rollin\'", DxAve, DyAve, Dxold
